...
void bonus::drowBonus(sf::RenderWindow* window, playground* field) {
	int width, height;
	width = height = 50;
...
void bomb::trigger(playground* field) {
	int t_x, t_y;
...
bool painter::GemIsBad(playground* field, int x, int y) {
	bool empty = false;

	if (x < field->GetGemAmountXY() && y < field->GetGemAmountXY()) {
...
void painter::trigger(playground* field){
